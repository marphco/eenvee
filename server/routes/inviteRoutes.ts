...
router.get("/:slug/invites", requireAuth, async (req: AuthRequest, res: Response) => {
  try {
...
router.post("/:slug/invites", requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    const { slug } = req.params;
    ...
    const newInvites = contacts.map((c: any) => ({
      eventSlug: slug,
      name: c.name,
      phone: c.phone,
      email: c.email,
