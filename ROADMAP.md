...
- [x] **Pagination DOM-aware per benedizione-nuziale**: bypass DOM-measurement (font async + verse fragmentato → sotto-stima). Char-cap manuale `cap=800` + `lineHeight=1.4` nel Body benedizione → 2ª formula entra in 2 pagine invece di 3. **Override "tieni R con strofa" line-aware**: soglia 24 righe visive, oltre la quale R va a pagina successiva (4ª formula verse-heavy = 26 righe altrimenti clipping).
- [x] **Letture e salmi cleanup massivo** (Python script): puliti tutti i 16 salmi (titoli/incipit/ritornelli con junk text rimosso, paragrafazione `testoCompleto` corretta), fix Sal 148 doppia R., XOR mode lettura/salmo/vangelo (preset OR custom, no entrambi). Salmo "consigliato" simmetrico: scelta lettura suggerisce salmo; scelta salmo suggerisce letture abbinate. Auto-popola ritornello su scelta salmo.
- [x] **Canti — solo pubblico dominio**: rimossi tutti i canti contemporanei italiani SIAE-protected (Frisina, Sequeri, Argüello, Taizé, ecc.) — non ridistribuibili da piattaforma SaaS senza licenza. Restano 12 canti gregoriani/latini classici **tutti con testo integrale**: Veni Creator, Alleluia gregoriano, Ubi caritas, Panis Angelicus, Adoro te devote, Tantum ergo, Pange lingua, Ave Maria (latino/Schubert/Gounod), Salve Regina, Magnificat. Dropdown canto: 3 modalità (con testo / solo titolo / personalizzato).
- [x] **Bold automatico nomi sposi** in tutti i Body via `BoldNamesContext` + `formatBodyText` con regex Unicode word-boundary. Sostituisce match interi (no parziali).
- [x] **Rimossa label "NOMI DEGLI SPOSI"** dal cover-editor: due label espliciti "NOME DELLO SPOSO" / "NOME DELLA SPOSA" con placeholder "es. Marco" / "es. Sara". Frecce su/giù allineate alla prima riga. Stessa modifica sia in sidebar che in `LibrettoEditorModal`.
- [x] **Rimossa sezione "Aggiungi pagina"** sia dalla sidebar (panel append-end) che dal modal contestuale (`AddPageAfterControl`): il template CEI default copre l'intero rito, l'utente non ha necessità di pagine custom aggiuntive. Riordino pagine esistenti resta possibile.
- [x] **Layout polish**: rimosso h1 "Prima formula"/"Seconda formula" dalla benedizione (subtitle "Benedizione Nuziale" sufficiente); margini compatti su Riti di Conclusione (marginBottom benedizioni 14→8, ℟ Amen marginTop 4→1, divider margin 12→6, lineHeight 1.55→1.4) per evitare clip della 1ª formula. Dropdown formula senza parentesi `(n. X)` — solo "Prima formula"/"Seconda formula"/etc.
- [x] **Animazione cover-flip realistica**: la copertina ora si sfoglia come un vero libro che si apre — `bookOpen` state + `coverTransition` machine (`'opening'|'closing'|null`), translate X del libro intero verso destra man mano che la copertina ruota attorno al bordo sinistro (= dorso). Pagina interna sinistra del back-cover blank già esistente fa da "dietro la copertina". `COVER_FLIP_MS=700` per un'animazione morbida. Bottone "Chiuso/Apri libro" commuta lo stato.
- [x] **Page-flip glitch — fix definitivo (3 cause concatenate)**: (1) `PageSlot` era inline dentro `LibrettoBooklet` → React ricreava componente ad ogni render e tutti i PageSlot facevano unmount/remount durante il flip → glitch della pagina sotto. **Hoistato a module-level + `React.memo`**. (2) Il flip-element veniva montato/smontato ad ogni flip causando lag visivo. **Architettura due flippers always-mounted** (forward + backward) con `useMotionValue` + `fmAnimate()` imperativo per controllo deterministico. (3) Il content delle pagine si aggiornava simultaneamente al flip → frame sbagliato visibile. **`contentIdx` lagged state** via `useEffect` che aggiorna solo `!flipping` → re-render PageSlot solo dopo che flip è finito.
- [x] **Animazione skip su click compulsivi (fix race condition)**: con `<motion.div animate={{rotateY}}>` declarativo + `transition.duration` dinamico, framer-motion poteva usare `duration: 0` stale per la nuova animazione → snap istantaneo. Risolto refactor a `useMotionValue` + chiamata esplicita `fmAnimate(motionValue, target, {duration})` che è deterministico.
- [x] **Drag&drop sfoglio rimosso**: era glitchato (mostrava 2 pagine simultanee, partiva da rotateY=0 anche post-drag). Sfoglio ora solo via click frecce/tasti freccia. Frecce laterali a -44px rimosse (causavano scrollbar orizzontale); navigazione pulsanti centrali sotto al libro.
- [x] **Scrollbar fix**: `overflow: hidden` sul container `perspective` (no `overflowX: hidden` che per CSS spec forza `overflow-y: auto` causando scrollbar verticale).
- [x] **Modal editor UX**: bottone Undo top-right con `historyRef` + debounce 500ms su `libretto.pages/cover/style`; pattern "Sicuro?" per delete pagina (3s timeout) al posto di `alert()`; colonne invertite (preview 480px, form `flex: 1` per più spazio editoriale); riordino pagine in TOC indice via chevron up/down (escluse cover/indice).
- [x] **Sidebar canto semplificata**: rimossa "Etichetta della pagina" (ridondante col titolo); dropdown unica con `isPreset` check; campi titolo/testo visibili solo per modalità "Personalizzato". Voce "Personalizzato" senza "(scrivi tu)". Subtitle "Etichetta" non più renderizzato in `renderCanto`.
- [x] **Feature flag `HIDE_CANTI_PAGES`** in `utils/libretto/types.ts` + helper `filterVisiblePages()`. Nasconde tutte le pagine canto runtime (sia in editor TOC sia in `paginate.ts`) senza rimuoverle dal codice. Tutti i canti commentati nei template `con-messa`/`senza-messa`. Navigazione `goPrev/goNext` salta pagine canto.
- [x] **Bottone "Chiudi anteprima" visibile**: sostituito `<Button variant="subtle">` (translucido invisibile su overlay scuro) con pill bianco custom (`background:#fff`, `borderRadius:999px`, `boxShadow` flottante, hover `translateY(-1px)`).

### 07-05-2026: Libretto Messa — Letture CEI + nuovi tipi pagina + UX polish 📖✨
- [x] **DB letture CEI completo** estratto da liturgia.maranatha.it/Matrimonio: **16 letture AT** + 2 Tempo Pasquale + 12 NT + 22 Vangeli + 17 Salmi responsoriali con **testo CEI integrale** salvato in `client/src/utils/libretto/cei.ts` (~87KB). Parser Python ad hoc che gestisce HTML FrontPage (`<a name>` + Word cruft). Total **69 voci** con `testoCompleto`. [2026-05-07]
- [x] **Auto-pair lettura → salmo**: campo `salmoCorrispondente` in `ReadingOption` (numero del salmo abbinato sul lezionario CEI per ogni lettura AT). Quando l'utente sceglie una lettura AT, l'app preimposta il salmo abbinato sulla pagina successiva (solo se `salmoId` vuoto, no overwrite). Dropdown lettura mostra `(→ Sal 127)` accanto ad ogni AT. Hint sotto il select con titolo descrittivo + abbinamento.
- [x] **12 nuovi tipi pagina** dal pattern .doc reale: `memoria-battesimo`, `gloria`, `colletta`, `litanie-santi`, `orazione-offerte`, `rito-pace`, `agnello-dio`, `orazione-comunione`, `riti-conclusione` (3 benedizioni + congedo), `atto-matrimonio-civile` (art. 143/144/147), `dedica-iniziale` (citazione + autore), `racconto` (es. "Isola dei Sentimenti"). Template testuali CEI auto-popolati con `{sposo1}`/`{sposo2}` placeholder. Pattern unificato `useTemplate?: boolean + testoCustom?: string + titoloCustom?: string` per consistenza editor.
- [x] **Crop foto cover interattivo**: `PhotoCropEditor` in sidebar con drag-to-pan + slider zoom (1.0-3.0x). Stile slider allineato a PropertyPanel (`custom-slider` + linear-gradient track). Forma 2 opzioni (Cerchio/Rettangolo). Niente conflitto col drag del block (sta in sidebar, non sul canvas).
- [x] **Ordine elementi cover**: `cover.elementsOrder?: CoverElement[]` con frecce su/giù sui campi-stessi (foto, frase iniziale, nomi, data, chiesa). Renderer rispetta l'ordine, salta elementi vuoti.
- [x] **Libretto chiuso = monopagina**: stato `bookOpen` + `closedDesktop` ramo dedicato (singola A5 portrait centrata, ombra "libro chiuso"), apertura via `AnimatePresence` + fade/scale (no più scatto istantaneo). Pagina interna sinistra blank (back-of-cover) invece di ripetere la copertina.
- [x] **Hint canti**: alert friendly nel PageEditor canto: "se non vuoi questo canto, elimina la pagina; testi spesso protetti da SIAE — meglio solo titolo".
- [x] **Frase iniziale centrata**: fix `<p>` con `maxWidth: 80%` + `margin: 0 auto` (era left-aligned per default block).
- [x] **Icon-box header square**: `borderRadius` da `50%` a `14px` per allineamento con TableauWidget.
- [x] **Tabs sidebar centrate**: padding override `'7px 4px'` + `minWidth: 0` per evitare che intrinsic content widths annullino la distribuzione `flex: 1` (stesso pattern di PaymentSection).
- [x] **Glitch hover risolto**: rimosso `onMouseEnter/Leave` con mutazione DOM diretta; guard funzionale su `setIsMobile`; `React.memo` su `LibrettoBooklet` e `LibrettoPageRenderer`.
- [x] **Pubblicazione visibile su mobile**: tableau e libretto rimossi da `useAbsoluteWidgetBranch` exclusion; `accentColor`/`sectionBg` ora passati nella public view (fix `--accent-rgb` undefined → rgba "trasparente").
- [x] **Tableau bug fix coordinati**: auto-assign manuali rispetta vincoli `avoid` E `together`; modali mobile non più clippati (`90dvh` + `safe-area-inset`); ricerca con disambiguazione 3-state (empty/not_found/ambiguous/resolved) che mostra full table cards con guest list highlighted; copy semplificato (TAVOLO X / Nome / [lista ospiti]); rimossa stella ⭐ + icona mappa; sfondo opaco bianco + blur 4px sui tavoli durante ricerca; `autoComplete="off"` su input.
- [x] **Editor modal full-screen** `LibrettoEditorModal` per modificare il libretto (workflow guidato wizard-lineare). Sostituisce le tab cover/pagine della sidebar, che diventa snellita: status chip + bottone primario "Modifica libretto" + Anteprima + Pubblica + sezione Stile (colori/font). [2026-05-07 r6]
  - Layout desktop: 3 colonne (TOC sx 260px · preview pagina centrale · form editor dx 420px) + top bar + bottom nav. Top bar: ✕ chiudi · titolo · jump-to-page input numerico cliccabile · indicatore autosave. Bottom nav: ←Indietro / Avanti→ + status.
  - Layout mobile: full-screen, single-column stacked (preview top 40vh · form scrollable bottom). Top bar mostra bottone "📑 Indice" che apre bottom-sheet drawer con TOC.
  - **TOC** raggruppa pagine per sezione liturgica (apertura · introduzione · parola · matrimonio · eucaristica · conclusione · extra), sezioni collassabili. Ogni voce ha pallino di stato: ● corrente (azzurro accent) · ● modificata (verde) · ● default (grigio chiaro). Click pagina = jump immediato.
  - **Wizard lineare**: bottoni Indietro/Avanti attraversano TUTTE le pagine in ordine (no auto-skip — utente vede ogni pagina anche quelle con testo standard CEI, può saltarle via TOC se preferisce). Tasti freccia (← →) supportati. ESC chiude modal.
  - **Jump-to-page**: input numerico in top bar, click sul numero attiva input editabile, Invio = jump. Supporta keyboard nav.
  - **Autosave silenzioso** ogni modifica (debounce 200ms su `libretto.pages/cover/style`). Indicatore "✓ Salvato" verde nell'angolo top-right.
  - **PageEditor riusato**: estratto + esportato `PageEditor`, `describePage`, design tokens (`labelStyle`/`inputStyle`/`fieldGap`/etc) da LibrettoSidebar.tsx.
  - **CoverInlineEditor** dedicato dentro modal (campi base nomi/data/chiesa/frase) con hint che redirige alla sidebar canvas per il crop foto avanzato.
  - **Add-page-after**: bottone "Aggiungi dopo questa" in fondo al form, inserisce nuova pagina dopo quella corrente e auto-jumpa.
- [x] **Pagina Indice** (`type: 'indice'`): nuovo tipo pagina auto-popolato dalle pagine del libretto. Campi: `profondita: 'macro'|'dettaglio'` (default macro = lista 5-6 sezioni; dettaglio = ogni pagina), `mostraNumeri` (default true), `titoloCustom`. Renderer ad hoc con righe titolo + dotted-leader + numero pagina. Aggiunto al template default `con-messa` E `senza-messa` come **seconda pagina** subito dopo cover. Esclude cover/indice/photo dal proprio elenco.
- [x] **Sezioni liturgiche** (`SezioneLiturgica`): nuovo campo opzionale `sezione?` su `PageBase`, derivato auto da `type` via `SEZIONE_DEFAULT_BY_TYPE` mapping (no migrazione richiesta). Helper `resolveSezione(p)`. Usato sia dal renderer indice che dal TOC del modal editor.
- [x] **4 letture Apocalisse aggiunte a mano** (Ap 5, 8-10 / Ap 19, 1.5-9a / Ap 21, 1-5a / Ap 22, 16-17.20) recuperate dal parser (testi presenti, ref/titolo erano vuoti per pattern prefix specifico). Tutte con `salmoCorrispondente` (32 o 44).
- [x] **Default template `con-messa` esteso** ai nuovi tipi pagina: 27 pagine ordinate per sezione liturgica reale (Riti Introduzione → Liturgia Parola → Liturgia Matrimonio → Liturgia Eucaristica → Riti Conclusione). Include memoria-battesimo, gloria, colletta, litanie-santi, orazione-offerte, rito-pace, agnello-dio, orazione-comunione, riti-conclusione, atto-matrimonio-civile (chi non vuole li elimina). Variante `senza-messa` invariata (più snella).
- [x] **Drag-to-flip pagine** desktop: pointerdown su lato destro pagina = forward, lato sinistro = backward. Tracciamento delta orizzontale → rotazione live (0° → ±180°) sulla pagina che si solleva, con stesso layout 3D del flip esistente (front/back facce + ombre direzionali). Release oltre soglia 30% (60°) → trigger goForward/goBack. Release sotto soglia → snap a 0. Mobile usa swipe esistente (no live visual, da fare in v2). Closed-cover esclusa (opening è transizione AnimatePresence già fluida).

### 06-05-2026: Libretto Messa — Add-on €15 ✨📖
- [x] **Widget Libretto Messa sfogliabile**: nuovo block-type `libretto` parallelo al Tableau, add-on €15 una tantum. Sfogliabile come libro vero (page-flip 3D con Framer Motion, mobile = swipe, desktop = due pagine affiancate + frecce + tasti freccia). Nessuna libreria esterna oltre quelle già nel bundle (+~2KB gzipped totali). [2026-05-06]
- [x] **DB letture CEI** (`client/src/utils/libretto/cei.ts`): metadata di tutte le opzioni del Lezionario CEI per il Matrimonio (9 AT, 7 Salmi, 12 NT, 9 Vangeli). Solo `{id, ref, titolo, incipit, popolare}` — niente testo completo nel DB per evitare problemi copyright e inaccuratezze; gli sposi possono incollare il testo intero nell'editor o lasciare solo il riferimento (formato comune nei libretti reali). 17 canti suggeriti per i momenti standard (ingresso/offertorio/comunione/ecc.).
- [x] **Testi liturgici fissi** (template parametrizzati): interrogazioni, consenso (forma dichiarativa + interrogativa), benedizione anelli — auto-popolati con i nomi degli sposi via `fillSpouses({sposo1}, {sposo2})`.
- [x] **Modello dati** (`utils/libretto/types.ts`): `LibrettoData = {variant: con-messa | senza-messa, cover, pages: LibrettoPage[], style, isPublished}`. 16 tipi pagina discriminati (cover/intro/lettura/salmo/vangelo/canto/interrogazioni/consenso/anelli/preghiere-fedeli/padre-nostro/benedizione-nuziale/dedica/ringraziamenti/photo/custom). Pagine drag-drop riordinabili.
- [x] **Template default** (`utils/libretto/templates.ts`): variante "con Messa" (18 pagine standard precompilate), variante "senza Messa" (15 pagine snelle). Persistite al momento della creazione del blocco per evitare ID instabili tra mount.
- [x] **LibrettoWidget**: pattern paywall identico al Tableau — editor sempre visibile, public view con overlay "Modulo Premium" se non pagato, "in preparazione" se pagato ma non pubblicato, sfogliabile completo se pubblicato.
- [x] **LibrettoBooklet sfogliabile**: animazione `rotateY` 3D con `perspective: 1500px`, ResizeObserver per switch desktop↔mobile dinamico, gestione touch swipe + tasti freccia + click su frecce laterali. Frame con sfondo personalizzato + ombre asse rilegatura + numerazione pagine.
- [x] **LibrettoSidebar** (desktop + mobile compact): tab Copertina/Pagine/Stile + Pubblicazione + Paywall. Lista pagine con sposta su/giù + elimina + toggle modifica inline. PageEditor con form contestuale per ogni tipo di pagina (lettura → dropdown CEI con star ⭐ sui popolari, canto → datalist suggerimenti, preghiere-fedeli → array intenzioni, ecc.). Preview anteprima sfogliabile in modal full-screen.
- [x] **Editor mobile in MobileToolbar** (compact): 4 tab (Copertina/Pagine/Stile/Pubblica) + paywall icona singola "Attiva" se non acquistato — replica esatta del flusso Tableau mobile per consistenza UX. Stesso pattern `compact + section` per riusare il codice del desktop.
- [x] **Server `subscriptionRoutes.ts`**: nuovo `kind: "libretto_addon"` (€15 = 1500 cents) in `create-unlock-intent` e `complete-unlock-intent`. Su success setta `addons.libretto = true` con `markModified`. Endpoint mock `/:slug/success-libretto-mock` per dev.
- [x] **`EventPurchaseModal`**: esteso `purchaseType` con `libretto_addon`, `LIBRETTO_ADDON_EUR = 15`, titoli "Attivazione Libretto Messa".
- [x] **Wiring**: `BLOCK_LIBRARY` non toccato (i widget si aggiungono via PageSection.tsx come gli altri); pulsante "Sezione Libretto Messa" nel pannello aggiungi sezione; `SectionCanvas` rendering editor + posizione draggable (`librettoX`/`librettoY`); `EventPublic` rendering pubblico (mobile 1:1 desktop); `BuilderSection` toolbar widget label "Libretto Messa".
- [x] **Note**: niente PDF stampa in v1 (deciso fuori scope), solo italiano (no latino), foto disponibili ma non nei template default. Build verde, TS strict pulito (exactOptionalPropertyTypes), lint zero errori sui nuovi file (sub-componenti dichiarati a livello modulo per regola `react-hooks/static-components`).
...
### 🔒 Lotto: Lock root widget — blocchi non-canvas non selezionabili
*Tutti i widget eccetto la sezione vuota (canvas libero) hanno un layout auto-dimensionante che non beneficia della selezione/spostamento del block-root. Renderlo selezionabile genera affordance fuorvianti (l'utente prova a trascinarlo, non succede nulla) e abilita per errore "aggiungi testo/immagine" che non hanno senso dentro un widget chiuso (RSVP, Gallery, Video, Map, Payment, Tableau, Libretto).*

- [ ] **Audit punti di selezione** root-blocco non-canvas: `SectionCanvas`, `BuilderSection`, `MobileToolbar`, hook `useEditorInteractions`/`useLayerManager`, `widgetLayerId.ts`. Identificare tutte le code-path che marcano selezionato un block-root.
- [ ] **Disabilita selezione root** per `block.type !== 'canvas'`: il click su widget non deve impostare `selectedLayerId` sul block-root. Resta selezionabile solo il widget interno (es. `widget-<blockId>`) per editing dei suoi campi.
- [ ] **Toolbar desktop blocco selezionato**: nascondere CTA "Aggiungi testo"/"Aggiungi immagine" quando il blocco selezionato è non-canvas. Mantenere solo le azioni applicabili (es. duplica/elimina sezione, cambia colore di sfondo).
- [ ] **Mobile toolbar(s)**: stessa regola. Audit di tutte le toolbar mobile (`MobileToolbar.tsx`, eventuali toolbar contestuali per widget, compact-mode tableau/libretto).
- [ ] **Widget renderer**: aggiungere flag `lockRoot: true` (o equivalente) ai renderer dei widget non-canvas così che `SectionCanvas` non offra outline/handle di drag sul block-root. Canvas vuoto resta `lockRoot: false`.
- [ ] **Smoke test desktop + mobile** per ogni widget: RSVP, Gallery, Video, Map, Payment, Tableau, Libretto. Confermare 1:1 editor↔public.
...
- [x] Libretto messa sfogliabile (block-type `libretto`, add-on €15, DB CEI, 16 tipi pagina, page-flip 3D Framer Motion, paywall Stripe, mobile editor compact mode) — 2026-05-06
- [ ] Timeline evento
- [ ] Menu catering
